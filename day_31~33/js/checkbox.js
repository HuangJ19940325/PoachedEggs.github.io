function genGroupCheckBox(wrapper, checkBoxArr) {
    ...
    let selectAll = "<div><input type='radio' checkbox-type='all'>全选</div>"
    // 遍历参数对象 
    let chidrenBox = [];
    for (let i = 0; i < checkBoxArr.length; i++) {
        // 生成各个子选项checkbox的html，给一个自定义属性表示为子选项
        let checkbox = "<div><input type='checkbox' checkbox-type='single' ";
        if (i == 0) {
            checkbox = checkbox + "checked='true'";
        }
        let checkboxVale = "value='" + checkBoxArr[i].text + "'>" + checkBoxArr[i].text + "</div>";
        checkbox = checkbox + checkboxVale;
